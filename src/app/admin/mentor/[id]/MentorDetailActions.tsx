...
import { updateMentorStatus } from "../../actions";
import { useRouter } from 'next/navigation';
import { CheckCircle, XCircle, Loader2 } from 'lucide-react';
import ConfirmationModal from '@/components/ConfirmationModal';

export default function MentorDetailActions({ id, currentStatus }: { id: string, currentStatus: string }) {
    ...
    const handleUpdate = (status: 'approved' | 'rejected') => {
        ...
        setModal({
            isOpen: true,
            title: isApprove ? "Setujui Mentor" : "Tolak Mentor",
            message: isApprove 
                ? "Apakah Anda yakin ingin menyetujui mentor ini?" 
                : "Silakan berikan alasan penolakan untuk mentor ini:",
            ...
            onConfirm: async (val?: string) => {
                ...
                setLoading(true);
                try {
                    await updateMentorStatus(id, status, isApprove ? undefined : val);
                    router.refresh();
                    closeModal();
                } catch {
                    alert("Gagal mengupdate status mentor.");
                } finally {
                    setLoading(false);
                }
            }
    ...
    return (
        <div className="space-y-3">
            ...
            >
                <CheckCircle size={20} />
                {currentStatus === 'approved' ? 'Mentor Disetujui' : 'Terima Mentor (Approve)'}
            </button>
            ...
            >
                <XCircle size={20} />
                {currentStatus === 'rejected' ? 'Mentor Ditolak' : 'Tolak Mentor (Reject)'}
            </button>
            ...
            <ConfirmationModal
                ...
                inputType="textarea"
                inputPlaceholder="Alasan penolakan mentor..."
                inputValue={reason}
